			...
			console.log("data data",jsonData);
			 ...
			 if (Object.keys(jsonData).length === 0 && jsonData.constructor === Object) {
			        ...
			        Object.keys(jsonData).forEach((key, index) => {
				        ...
				        //if( self &&((item["finance_status"]==="settled")||(item["finance_status"]==="rejected"))){
				        	if(item["empId"]===empId){
				        		count++;
				        		 if(item["self"]==="no"){
				 	            	status = "admin_status";
			           ...
			           // row += "  <td>" + item["name"] + "</td>";
			            ...
			            if(item["self"]==="yes"){
				            if(item["finance_status"]==="settled"){
				            	 row += "  <td>" + item["finResponseOn"] + "</td>";
				            	
				            	row += " <td>";
				            	row += "   <button class='btn btn-success btn-sm Approve-btn' data-empid='"+ item["empId"] +"' data-key='"+ key+"'>Settled</button>";	
				            }
			            ...
			            }else if(item["self"]==="no"){
			            	if(item["admin_status"]==="approved"){
			            		row += "  <td>" + item["adminResponseOn"] + "</td>";
			            		row += " <td>";
