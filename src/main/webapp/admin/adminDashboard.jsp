...
<div class="main-content">
    ...
    <div class="containerDiv">
        <div class="graphDiv">
            ...
            <div class="graph1">
                <canvas id="myChart1"  width="230" height="230"></canvas>
                <p style="margin-bottom:0px">Users</p>
                
	                <p style="display:none" id="p1">No Records Found </p>&nbsp;&nbsp;
        ...
        <div class="requestDiv">
            <table class="table table-striped table-light table-borderless col-12">
                ...
                <tbody class="table-class" id="request-table">
            ...
            <div id="noDataMessage" style="text-align: center; margin-top: 20px;">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p>Do you want to <span id="modalAction"></span> this application? </p>
...
$(document).ready(function(){
	...
	var countInactive =0;
		...
		console.log("kjbfedbjdsbjsbjdsbjsaooano");
		 ...
		 $.ajax({
			 ...
			 success: function(response){
				 ...
				 Object.keys(response).forEach(function (key) {
					 if(response[key].self ==="no" && response[key].manager_status==="approved"){
						 
						 var admin_status = response[key].admin_status;
						 var manager_status = response[key].manager_status;
						 if(admin_status === "approved" && manager_status === "approved") countAccept++;
						 else if(admin_status === "rejected" && manager_status!=="rejected") countReject++;
						 else if(admin_status === "pending" && manager_status === "approved") countPending++;
					 	}
				 })
				 Object.keys(users).forEach(function (key) {
					 	 console.log("keys::",key);
						 var active = users[key].active;
						 
						 if(active === "yes") countActive++;
						 else if(active === "no") countInactive++;
						
				 	})
				//console.log("New countAccept : ", countActive);
				 	
				 //console.log("after the function call");
				 //console.log("active:::",countActive, countInactive);
				 if(countAccept == 0 && countReject == 0 && countPending==0){
					 document.getElementById("p0").style.display = "inline";
					 //document.getElementById("no-records-row").style.display = "inline";
					$("noDataMessage").show();
					 document.getElementById("myChart").style.display = "none";
				 }
				 
				 console.log("jsonData inside here",response);
				
				// if (Object.keys(response).length === 0 && response.constructor === Object) {
					//    console.log("The JSON object is empty 1.");
					    //document.getElementById("no-records-row").style.display = "inline";
					 //   tableEmpty();
				...
				tableData(response);	
				 //console.log("hereadasdasasd",countAccept, countReject, countPending);
				 //console.log("active:::",countActive, countInactive);
				 loadInsights();
				 loadInsights1();
			 },
	 ...
	 function requestChart(){
			$.ajax({
				 ...
				 success:function(response){
					 countActive = 0;
					 countInactive =0;
					 console.log("response after the function",response);
					 Object.keys(response).forEach(function (key) {
					 
					 
						 console.log("keys::",key);
						 var active = response[key].active;
						 
						 if(active === "yes") countActive++;
						 else if(active === "no") countInactive++;
						 
						 //else if(admin_status === "pending") countPending++;
						 
					 	
				 	})
				 	console.log("active::",countActive,"inactrive",countInactive);
	...
	function tableData(jsonData) {
	    ...
	    let count = 0; // Initialize count to track rows added
	    ...
	    if (Object.keys(jsonData).length === 0 && jsonData.constructor === Object) {
	        ...
	        $("#noDataMessage").hide(); // Hide no data message if data exists
	    }
	    ...
	    Object.keys(jsonData).forEach((key, index) => {
	        ...
	        if (item['manager_status'] === "approved" && item['self'] === "no" && item['admin_status'] === "pending") {
	            count++; // Increment count for valid rows

	            let totalAmount = 0;
	            const length1 = item.Destination.length;

	            for (let i = 0; i < length1; i++) {
	                totalAmount += parseFloat(item.amount[i] || 0); // Calculate total amount
	            }

	            // Main row with class "main-row"
	            let row = "<tr class='main-row'>";
	            row += "  <td>";
	            row +=
	                "<a href='#' style='text-decoration:none;color:#000;' class='toggle-collapse' data-toggle='collapse' data-target='#" +
	                collapseId +
	                "' role='button' aria-expanded='false' aria-controls='" +
	                collapseId +
	                "'>";
	            row +=
	                "<i class='toggle-icon fas fa-chevron-down' id='icon-" +
	                index +
	                "'></i>&nbsp;&nbsp; " +
	                key;
	            row += "</a>";
	            ...
	            row +=
	                "   <button class='btn btn-success btn-sm Approve-btn' data-empid='" +
	                item["empId"] +
	                "' data-key='" +
	                key +
	                "' data-amount='" +
	                ...
	                "'>Accept</button>";
	            row +=
	                "   <button class='btn btn-danger btn-sm Reject-btn' data-empid='" +
	                item["empId"] +
	                "' data-key='" +
	                key +
	                "' data-empname='" +
	            ...
	            // Append row to the main table body
	            $("#request-table").append(row);
	        }
	    });
	    ...
	    // If no rows were added, show the noDataMessage
	    if (count === 0) {
	        $("#noDataMessage").show();
	    } else {
	        $("#noDataMessage").hide();
	    ...
	    $("#confirmAction").on("click", function () {
           ...
           var formattedDate = date.toLocaleString('en-GB', { 
            ...
            if(action === "APPROVE"){
            	...
            	console.log("inside the actio settle");
            	console.log("sdfda:",key,action,name,amount,remarks);
            	$.ajax({
            	...
            	console.log("inside the actio reject remarks",remarks);
            	$.ajax({
                    url: 'SendMailServlet', // Servlet URL for updates
                    method: 'POST',
                    //contentType: 'application/json',
	...
	let myChart = null; // Declare the global variable to hold the chart instance
	 function loadInsights() {
		 ...
		 console.log("entering into load insights");
				...
				console.log("count of application:::",countPending, countAccept, countReject);
	             // Re-create the chart
	 ...
	 let myChart1 = null; // Declare the global variable to hold the chart instance
	 function loadInsights1() {
	     
	             if (myChart1 != null) {
	                 myChart1.destroy(); // Destroy the existing chart before creating a new one
	             }
				 console.log("entering into load insights1");
	             ...
	             // Clear the canvas before creating a new chart
	             ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
				console.log(countActive, countInactive);
	             // Re-create the chart
