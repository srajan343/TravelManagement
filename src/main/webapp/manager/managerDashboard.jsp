...
$(document).ready(function(){
	...
	// async function loadRequests(){
		...
		function loadRequests(){
		 ...
		 $.ajax({
			 ...
			 success:function(response){
				 ...
				 countReject = 0;
				 countPending = 0;
				 Object.keys(response).forEach(function (appKey) {
		                var app = response[appKey];
		                
		                // Check if this application belongs to an employee whose manager is the logged-in manager
		                if (app.manager_id === empId) {  // Only consider applications where manager_id matches empId
		                    var manager_status = app.manager_status;
		                    
		                    // Count the applications based on the manager status
		                    if (manager_status === "approved") countAccept++;
		                    else if (manager_status === "rejected") countReject++;
		                    else if (manager_status === "pending") countPending++;
		                }
		            });

		            // Show the counts (this is where you show the number of approved, rejected, and pending applications)
		            console.log("Accepted:", countAccept, "Rejected:", countReject, "Pending:", countPending);
		            ...
		            // Call function to load insights based on data
		            loadInsights();
		        },
		...
		function tableData(jsonData) {
		    ...
		    var empId = "<%= session.getAttribute("empId") %>";
		    var count = 0; // Initialize count to track matching rows
		    ...
		    if (Object.keys(jsonData).length === 0 && jsonData.constructor === Object) {
		        ...
		        Object.keys(jsonData).forEach((key, index) => {
		            ...
		            if (item.manager_id === empId) {
		                if (item["manager_status"] === "pending") {
		                    count++; // Increment count for each matching row
		                    console.log("respone",jsonData);
		                    console.log("modalcustomerName",item["customer_name"]);
		                    var totalAmount = 0;
		                    ...
		                    row += "   <button class='btn btn-success btn-sm Approve-btn' data-date='" + item["date"] + "' data-empid='" + item["empId"] + "' data-key='" + key + "' data-amount='" + totalAmount.toFixed(2) + "' data-empname='" + item["name"] + "' data-custname='"+item["customer_name"]+"' data-source='"+item["source"]+"'>Accept</button>";
		                    row += "   <button class='btn btn-danger btn-sm Reject-btn'    data-empid='" + item["empId"] + "' data-key='" + key + "' data-empname='" + item["name"] + "' data-amount='" + totalAmount.toFixed(2) + "' data-custname='"+item["customer_name"]+"' data-source='"+item["source"]+"' data-date='" + item["date"] + "' >Reject</button>";
		                    row += " </td>";
		                    row += " </tr>";
		    ...
		    // Show or hide the "No Recent Application Received" message
		    if (count === 0) {
		        $("#noDataMessage").show();
		    } else {
		        $("#noDataMessage").hide();
	 ...
	 $(".table-class").on("click",".Approve-btn",function(){
         ...
         //loadRequests();
          ...
          $("#modalEmployeeName").text(name);
               ...
               $("#modalCustomerName").text(custName);
               $("#modalDate").text(date);
	 ...
	 //after confirming the data 
	    $("#confirmAction").on("click", function () {
            ...
            var custName = $("#modalCustomerName").text();
